...
M.setup = function(client, buffer)
  local mappings = {
    ...
    ["b"] = {
      ...
      ["c"] = { "<CMD>lua lsb.code_action()<CR>", " Code action" },
      ["f"] = { "<CMD>lua lsb.formatting_sync()<CR>", "ﯕ Format" },
      ["ff"] = { "<CMD>lua lsb.formatting()<CR>", "ﯕ Format" },
    ...
    ["g"] = {
      ...
      ["d"] = { "<CMD>lua lsb.definition()<CR>", " Goto definition" },
      ...
      ["c"] = { "<CMD>lua lsb.clear_references()<CR>", " Clear References" },
    },
    ["w"] = {
      ["name"] = " Workspaces",
  ...
  if bo.filetype == "java" then
    mappings["j"] = {
      ...
      ["u"] = { "<CMD>JDTUpdateProjectConfig<CR>", "ﮮ Update project config" },
      ["j"] = { "<CMD>JDTJol<CR>", " JOL" },
